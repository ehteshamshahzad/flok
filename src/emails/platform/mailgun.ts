...
export class MailgunEmail {
    ...
    async sendMailgun(emailDataDto: EmailDataDto) {
        ...
        const result = await client.messages.create('flokkids.com', {
            from: `${emailDataDto.senderName ? emailDataDto.senderName : 'Flok Kids'} <${this.configService.get<string>('FK_SOURCE_EMAIL')}>`, // sender address
            ...
            'h:Reply-To': `${emailDataDto.replyToAddresses && emailDataDto.replyToAddresses.length > 0 ? emailDataDto.replyToAddresses : []}`,
